

## II

**问2** 答案请全部从各题下方的选项中选择。

考虑直角三角形 \(ABC\)，其满足 \(\angle ABC=\dfrac{\pi}{2}\)。将该三角形的重心记为 \(G\)，并设为 \(AG=1\)。  
当 \(\angle BAC=\theta\) 时，研究 \(\theta\) 与线段 \(AG,\ BG,\ CG\) 长度之间的关系。

在复数平面上考虑。让重心 \(G\) 与原点重合，且表示点 \(A\) 的复数为 1。  
进一步，当把表示点 \(B,\ C\) 的复数设为 \(\beta,\ \gamma\) 时，考虑使 \(\beta\) 的虚部为正的三角形。

(1)　\(\angle ABC=\dfrac{\pi}{2}\) 因为是

\[
\left|\dfrac{\gamma-1}{\beta-1}\right|=\boxed{N}
\]

是。因此

\[
\dfrac{\gamma-1}{\beta-1}=\boxed{O}
\]

是。由此，得到关于 \(\beta,\ \gamma\) 的关系式 \(\boxed{P}\)。

**选项（⓪〜⑨）**  
⓪　\(\dfrac{1}{\sin\theta}\)　　①　\(\dfrac{1}{\cos\theta}\)　　②　\(\dfrac{1}{\tan\theta}\)　　③　\(\tan\theta\)  
④　\(\tan\theta+i\)　　⑤　\(1+i\tan\theta\)　　⑥　\(1-i\tan\theta\)  
⑦　\((\tan\theta+i)\beta-\gamma-\tan\theta+1-i=0\)  
⑧　\((1+i\tan\theta)\beta-\gamma-i\tan\theta=0\)  
⑨　\((1-i\tan\theta)\beta-\gamma+i\tan\theta=0\)





(2)　以后，设 \(\tan\theta=t\)。以 \(\beta\)、\(\gamma\) 为顶点的三角形的重心与原点一致，并且由关系式 \(\boxed{P}\)，\(\beta,\ \gamma\) 可用 \(t\) 表示。

\[
\beta=\boxed{Q},\qquad \gamma=\boxed{R}
\]

被表示为。因此

\[
BG=\boxed{S},\qquad CG=\boxed{T}
\]

是。

**选项（⓪〜⑨）**  
⓪　\(-\dfrac{1+2ti}{2+ti}\)  
①　\(-\dfrac{1+ti}{2-ti}\)  
②　\(-\dfrac{1+ti}{1+2ti}\)  
③　\(\dfrac{-1+ti}{2+ti}\)  
④　\(\dfrac{-2+ti}{1+2ti}\)  
⑤　\(\dfrac{-1+2ti}{2-ti}\)  
⑥　\(\sqrt{\dfrac{4+t^2}{1+t^2}}\)  
⑦　\(\sqrt{\dfrac{4+t^2}{1+4t^2}}\)  
⑧　\(\sqrt{\dfrac{1+4t^2}{4+t^2}}\)  
⑨　\(\sqrt{\dfrac{1+t^2}{4+t^2}}\)

(3)　根据(2)得到的结果

\[
\lim_{\theta\to+0} BG=\boxed{U},\qquad
\lim_{\theta\to+0} CG=\boxed{W},
\]

\[
\lim_{\theta\to\frac{\pi}{2}-0} BG=\boxed{V},\qquad
\lim_{\theta\to\frac{\pi}{2}-0} CG=\boxed{X}
\]

是的。另外，当\(BG=\dfrac{2}{3}\)时，为\(CG=\boxed{Y}\)。

**选项（⓪〜⑨）**  
⓪　\(0\)　　①　\(1\)　　②　\(2\)　　③　\(3\)　　④　\(4\)  
⑤　\(\dfrac{1}{2}\)　　⑥　\(\dfrac{1}{4}\)　　⑦　\(\dfrac{\sqrt{10}}{3}\)　　⑧　\(\dfrac{\sqrt{11}}{3}\)　　⑨　\(\dfrac{\sqrt{13}}{3}\)


