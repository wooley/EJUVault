...
const DEFAULT_OUTPUT_DIR = path.join(DEFAULT_CONTENT_DIR, 'index', 'solution_html');
...
function toSolutionRef(contentDir, filePath) {
...
async function main() {
  ...
  const solutionDir = path.join(contentDir, 'solutions');
  const indexPath = path.join(contentDir, 'index', 'solution_html_index.json');

  const files = listFiles(solutionDir, '.md');
  const solutions = [];
  ...
  for (const file of files) {
    ...
    solutions.push({
      solution_ref: toSolutionRef(contentDir, file),
  ...
  console.log(`Generated ${solutions.length} solution HTML files in ${outputDir}`);
